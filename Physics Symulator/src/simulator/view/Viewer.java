...
class Viewer extends SimulationViewer {
		...
		addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case 'h':
					...
					break;
				case 'v':
					_showVectors = (_showVectors ? false : true);
					repaint();
					break;
				case 'g':
					_selectedGroupIdx++;
	...
	protected void paintComponent(Graphics g) {
		...
		JLabel help1 = new JLabel("h: toggle help, v: toggle vectors, +: zoom-in, -: zoom-out, =: fit");
		add(help1);
	...
	private void showHelp(Graphics2D g) {
		...
		g.drawString("g: show next group", 10 , 55);
	...
	private void drawBodies(Graphics2D g) {
		...
		for(Body b : _bodies) {
			if (isVisible(b)) {
				...
				posX = _centerX - 2 + (int) ((b.getPosition().getX())/_scale);
				posY = _centerY - 2 - (int) ((b.getPosition().getY())/_scale);
				g.fillOval(posX, posY, 10, 10);

				if (_showVectors) {
					//VELOCIDAD
					auxX = b.getVelocity().direction().getX();
					auxY = b.getVelocity().direction().getY();
					
					flechaX = posX + (int) (20 *  auxX);
					flechaY = posY - (int) (20 *  auxY);
					
					drawLineWithArrow(g, posX + 5, posY + 5, flechaX + 5, flechaY + 5, 7, 4, 
							Color.GREEN, Color.GREEN);
					
					//FUERZA
					auxX = b.getForce().direction().getX();
					auxY = b.getForce().direction().getY();
