...
public class Main {
	...
	private static Options buildOptions() {
		...
		//mode
		cmdLineOptions.addOption(Option.builder("m").longOpt("mode").hasArg().desc("Possible values:"
				+ " 'batch' (Batchmode), 'gui' (Graphical User Interface mode). Default value: 'gui'.").build());
	...
	private static void startBatchMode() throws Exception {
		...
		//Crear fichero de entrada y fichero de salida
		InputStream i = new FileInputStream(new File(_inFile));
		OutputStream o;
		if(_outFile == null) {
			o = System.out;
		}else
		 o = new FileOutputStream(new File(_outFile));
		
		Controller controller = new Controller (ps, _forceLawsFactory, _bodyFactory);
		controller.loadData(i);
		controller.run(_steps, o);
	}
